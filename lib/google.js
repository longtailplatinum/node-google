...
var igoogle = function(query, sedb, lang, start, proxy, callback) {
    ...
    if (!locale) {
        ...
        newUrl += '&cr=country' + locale.cc.toUpperCase();
            console.log(newUrl)
        ...
        request(requestOptions, function(err, resp, body) {
            if ((err == null) && resp.statusCode === 200) {
                ...
                var links = []
                var totalPages = Number($('.g').length) + Number($(mapResults).length) + Number($(advertisements).length) + Number($(inTheNews).length) + Number($(peopleAsk).length) + Number($(sponsored).length);
