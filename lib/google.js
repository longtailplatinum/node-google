...
let GEOSURF_NO_PROXY_COUNTRIES = [ 'kh', 'bz', 'ee', 'fi', 'gt', 'ni', 'rw', 'ie', 'cd', 'cg'];
...
let COUNTRY_CODES_TO_UULE = [
    ...
    {cc: 'ao', tld: 'co.ao', name: 'Angola', uule: 'w+CAIQICIGQW5nb2xh'},
    ...
    {cc: 'cd', tld: 'cd', name: 'Democratic Republic of the Congo', uule: 'w+CAIQICIgRGVtb2NyYXRpYyBSZXB1YmxpYyBvZiB0aGUgQ29uZ28='},
    {cc: 'cg', tld: 'cg', name: 'Republic of the Congo', uule: 'w+CAIQICIVUmVwdWJsaWMgb2YgdGhlIENvbmdv'},
    {cc: 'ci', tld: 'ci', name: "Côte d'Ivoire", uule: 'w+CAIQICINQ290ZSBkJ0l2b2lyZQ=='},
    {cc: 'cm', tld: 'cm', name: 'Cameroon', uule: 'w+CAIQICIIQ2FtZXJvb24='},
    {cc: 'kh', tld: 'com.kh', name: 'Cambodia', uule: 'w+CAIQICIIQ2FtYm9kaWE='},
    {cc: 'cr', tld: 'co.cr', name: 'Costa Rica', uule: 'w+CAIQICIKQ29zdGEgUmljYQ=='},
    {cc: 'hr', tld: 'hr', name: 'Croatia', uule: "w+CAIQICIHQ3JvYXRpYQ0K"},
    ...
    {cc: 'lt', tld: 'lt', name: 'Lithuania', uule: 'w+CAIQICIFSmFwYW4='},
    ...
    {cc: 'rw', tld: 'rw', name: 'Rwanda', uule: 'w+CAIQICIGUndhbmRh'},
    ...
    {cc: 'ua', tld: 'com.ua', name: 'Ukraine', uule: 'w+CAIQICIHVWtyYWluZQ=='},
    ...
    {cc: 'lk', tld: 'lk', name: 'Sri Lanka', uule: 'w+CAIQICIJU3JpIExhbmth'},
    {cc: 'ng', tld: 'ng', name: 'Nigeria', uule: 'w+CAIQICIHTmlnZXJpYQ=='},
    {cc: 'pr', tld: 'com.pr', name: 'Puerto Rico', uule: 'w+CAIQICILUHVlcnRvIFJpY28='}
]
...
let igoogle = function(query, sedb, lang, start, proxy, cert, serpResults, callback) {
    let URL = '%s://www.google.%s/search?hl=%s&q=%s';
    if (GEOSURF_NO_PROXY_COUNTRIES.indexOf(sedb) > -1) {
        URL = '%s://www.google.%s/search?hl=%s&q=%s&uule=%s&ip=0.0.0.0&source_ip=0.0.0.0&ie=UTF-8&oe=UTF-8&noj=1&igu=1&nomo=1&nota=1&biw=1075&bih=5000&gl=%s';
    }
    ...
    if (!locale) {
        ...
        request(requestOptions, function(err, resp, body) {
            if ((err == null) && resp.statusCode === 200) {
                ...
                linkSelector = linkSel1;
                $(itemSelector).each(function(i, elem) {
                    if ($(linkSel1).length >= $(linkSel2).length) {
                        linkSelector = linkSel1
                    } else {
                        linkSelector = linkSel2
                    }
                    console.log("link selector", linkSelector)
                    let linkElem = $(elem).find(linkSelector)
                    ...
                    if (linkSelector == linkSel2) {
                        item.title = $(elem).find(`${linkSel2} h3`).first().text();
                    }
